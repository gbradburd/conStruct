...
There are 4 data objects you need to run a `conStruct` analysis:

1. [allele frequency data]

2. [sample size data]

3. [geographic sampling coordinates]
...
You must specify a vector of sample sizes for your samples.
(Make sure the data are of class `vector`, and that it's not a `list`.)
The sample size gives the number of chromosomes genotyped in that sample.
So, a successfully genotyped diploid individual would have a sample size 
of 2, and a sample of 10 diploid individuals would have a sample size of 
20. If the numbers of successfully genotyped chromosomes in a sample are
highly variable across loci, you can give the average sample size across 
loci.

An example sample size data vector is included in the 
`conStruct.data` object included with the package.
...
# look at the sample size data 
#	for the first 5 populations
conStruct.data$sample.sizes[1:5]
```
