...
the \gc{main} difference is that our likelihood assumes that allele frequencies are normally distributed 
...
computational speed.)
\gc{The second small difference is that unlike the original STRUCTURE where
  allele frequencies at a locus are independently drawn for each
  cluster \citep{STRUCTURE}, our non-spatial model of allele
  frequencies at locus assumes that each layer has an allele frequency
  that deviates away ( due to drift) from some shared
  global allele frequency. Our model of allele frequencies is therefore more
  closely related to the F-model prior for allele frequencies of \citet{falush2003}.
