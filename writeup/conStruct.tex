...
Finally, we calculated layer importance for 
...
and so the blue layer may extend
through the intervening (unsampled) northern Great Plains and Canadian Shield, 
with the red layer presumably then stretching up into British Columbia.
...
it \gc{currently scales} poorly with number of samples.
...
\gc{Our speed, on large sample sizes, could be improved by adopting rank one updates to the
 inverse of the covariance matrix for updating an individual's
 admixture proportions, which alters only a single row/column of the
 covariance matrix. However, we have not implemented this yet as it
 would likely mean losing our Hamilton MCMC for our parameters.}

%The other main difference is the model of noise,
%but it is unclear what effects this may have,

\gc{SO I THINK IF YOU SAY THIS you need to give some run time specs,
  mainly to show that it's not insanely slow.}

%Binomially clearly is, genotypes are discrete:  "or indeed, which is closer to the truth."
...
\gc{we emphasize that there} is no true best value of $K$.
\gc{The choice} of best $K$ is always relative to the data in hand.
...
(\gc{conceptually similar to} taking successively shallower
cross-sections of the population phylogeny),  %%note that the
                                %%equivalent would only be ~true if we
                                %%had same sample size from each
                                %%proportion.
...
\gc{In adding to spatial realism to the groups used by clustering methods,
it is important to re-emphasize that that clusters/layers detected by
these methods do not necessarily accord to distinct, ancestral
populations. Nor does a non-zero admixture proportion indicate that admixture (i.e. gene
flow) must have occurred. Both groupings and admixture proportions
should be viewed as hypotheses that should be subject to futher
testing, see \citet{Falush:16} for futher discussion of these points. }
...
This ``lumping" might better reflect biological \gc{``reality'',  
...
may be quite genetically diverged from one another, \gc{perhaps especially at
  loci underlying adaptive traits}, 
...
the sampled organisms experience (or have experienced) \gc{dispersal
  on } their landscape.
