...
\gc{Each of these methods perform best in particular situations,
but many can give misleading results when applied to data that show a continuous pattern of isolation by distance 
...
\gc{independent} lattice of demes,
...
(see Fig \ref{sim_setup}). \gc{We collapse the layers together using
random admixture proportions that are spatially autocorrelated  (see
{\bf XXXX} for details), so
that both the frequencies in clusters and the cluster memberships are
spatially autocorrelated.} We simulated datasets using $K=1$, 2, and 3 layers,
...
While it is true that each non-spatial cluster is \gc{genetically} more similar within itself
...
\gc{Even} when the spatial model is run with $K=7$, 
the inferred admixture proportions are nearly identical to 
those estimated under the true value of $K$ for each simulation.
\gc{Moreover, the method infers the true admixture proportions with high accuracy, tight precision, and good coverage 
(Figs \ref{simK2_adprop_fit} and \ref{simK3_adprop_fit}).}


\gc{Finally, we calculated} of layer importance for 
analyses run in each simulation (Fig \ref{simK1_laycon}, \ref{simK2_laycon}, and \ref{simK3_laycon}).
In the spatial analyses, \gc{once we pass the true $K$, subsequent
layers add little in terms of (co)variance explained}  %$K$ layers remains constant even with additional layers;
in contrast, additional layers in the nonspatial analyses continue to
be important for describing variation, \gc{as they are forced to partition
up the spatial patterns within clusters}.
...
Our data, \gc{from} \cite{geraldes_etal_2014}, 
...
All models with $K>1$ assigned the majority of each \gc{of the two} species
...
\gc{For values of $K<4$,} the spatial model was strongly preferred over the nonspatial model (Fig \ref{populus_xvals}).
...
\gc{Based on our cross-validation results we view $K=3$ spatial-model
  as sufficient description of the data.} 
...
cottonwood is absent \gc{(the break between yellow and red, for $K \geq 3$)} . 
...
were experimentally shown to display \gc{differences in ecologically relevant} phenotypes 
...
from which they subsequently expanded to occupy their current \gc{range
...
Bears in the sample are partitioned into two main groups \gc{($K \geq 2$)}:
one \gc{(red)} to the east of the Rocky Mountains, 
which also occurs in Alaska,
the other primarily west of the Rockies \gc{(blue)}.
\gc{The disjointed range of the blue layer, likely reflects the fact
that Canada was not sampled, and so the blue layer may extend
through the intervening (unsampled) northern Great Plains and Canadian
Shield, with the red layer presumably then stretching up into British Columbia.}
...
it is clear that the additional \gc{spatial} layers beyond $K=2$ are contributing only a very small amount toward 
...
\gc{make more detailed interpretations untrustworthy.}
...
(e.g., Fig \ref{bear_sp_pies:K5} vs \ref{bear_nsp_pies:K5}).
...
that relatedness ought to decay with \gc{geographic} distance, even within a population, 
...
\gc{and demonstrate that real datasets can better described using fewer
  layers, by acknowledging isolation by distance. }
...
\gc{will be common within} clusters.
...
At $K=3$, the southernmost populations of \textit{P. trichocarpa} are in the yellow cluster, 
