...
\newcommand{\responsefile}{response2reviews}  % name of the review responses file
...
e.g.,  
\cite{BEDASSLE,EEMS,barton2008,Ringbauer2018}.)
...
Spatial information has been incorporated into some of these methods, \revpoint{2}{5}
...
\cite{nagylaki1978diffusion,nagylaki1988,barton2002neutral},
but substantial work remains to be done \cite{barton2013modelling}. \revpoint{2}{8}
...
with spatially autocorrelated cluster membership \cite{Frantz2009,meirmans2012}. \revpoint{2}{21}
...
\burl{https://github.com/gbradburd/conStruct}. \revpoint{2}{22}
...
and $X_i = 0$ otherwise. \revpoint{2}{6}
Then, 
...
(to retain insensitivity to choice of reference allele). \revpoint{2}{7}
...
would be Wishart distributed with degrees of freedom equal to $L$, \linelabel{ll:wishart}
...
are also available at the same site. \revpoint{2}{11}
...
and collapsed the layers together using random, spatially autocorrelated admixture proportions.
See \secref{sim_details} for more details, including parameter values used. \linelabel{ll:sim_params_ref}
...
\linelabel{ll:admixture_sim_results}
...
\linelabel{ll:admixture_sim_xval}
...
\linelabel{ll:admixture_populus}
...
\paragraph{Comparison to ADMIXTURE} \linelabel{ll:admixture_bears}
...
Comparing results of the two methods, it is clear that even at $K=3$, 
ADMIXTURE invokes clusters to describe what seems to be, 
...
provide more information on ADMIXTURE's lack of fit. \revpoint{1}{4}
First, covariance between different sampling locations of Eastern bears
falls off with distance,
indicating a residual pattern best explained by isolation by distance within laters.
Second, the covariance between bears assigned to ADMIXTURE's gold and red layers
(the furthest Northwestern (Alaskan) bears and the Eastern bears, respectively)
appears to be a natural extension of the decay of covariance with distance,
falling only slightly lower values than covariances 
...
with a model of isolation by distance. \revpoint{2}{14}
...
\linelabel{ll:admixture_few_loci}.
...
which are often predicated on the identification of discrete ``management units'' (MUs) 
...
which is more strongly affected by densely sampled locations. \linelabel{ll:cov_config}
...
with the aim of equalizing variances across loci. \revpoint{2}{3}
...
\paragraph{Caveats and considerations} \linelabel{ll:caveats}
...
so differences in $\phi$ between layers should not be overinterpreted. \revpoint{2}{10}
Again, we encourage users to run analyses across multiple values of $K$, 
to examine the spatial covariance functions within layers when interpreting results.
...
We employ a Monte Carlo cross-validation approach for model comparison \cite{picard1984}. 
...
in a region of the genome that also has loci included in the testing partition. \revpoint{2}{19}
...
The parameters used to simulate the data were as follows: \linelabel{ll:sim_params}
