...
but substantial work remains to be done \cite{barton2013modelling}.
...
See \secref{sim_details} for more details, including parameter values used. \llname{ll:sim_params_ref}
...
\begin{figure}
	...
	showing maps of admixture proportions estimated using the nonspatial \texttt{conStruct} model for $K=2$ through 4 (top row)
	and the spatial \texttt{conStruct} model for $K=2$ through 4 (bottom row).
    As there is only a single layer in the simulation, the spatial model accurately depicts the data
...
\paragraph{Comparison to ADMIXTURE} \linelabel{ll:admixture_populus}
...
This led to better performance on test data.
