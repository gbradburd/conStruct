...
% across the range of a population or species, but more, it 
...
% by jointly accommodating both continuous and discrete patterns of differentiation. 
...
\plr{I felt like we said "continuous and discrete" too many times in the abstract.}
...
Descriptions of taxa using phylogenetic trees omit within-taxon variation,
especially when these are intermingling subgroups.
...
% (Analytical theory \citep{barton2002neutral} predicts the decay should be proportional to $\exp{-D}/\sqrt{D}$.)
...
In the methods section \ref{rationale} we lay out a
...
than others (see \secref{Xvalidation} of the appendix for more on our cross-validation procedure).
...
To do this, we use the following alternative description of our covariance model:
...
We collapsed the layers together using random admixture, spatially autocorrelated proportions 
...
and compared predictive performance of the models using cross-validation to compare the predictive performance.
...
\plr{rearranged this to focus more on take-homes}
...
spatial \texttt{conStruct} does not create spurious clusters when there are none:
...
when data is simulated with $K=1$ but analyzed with $K>1$ --
...
Each non-spatial cluster is in truth genetically more similar within itself
...
\plr{How do the indivs partition across the drainages?  Perhaps give range of number of indivs per drainage?}
...
\begin{figure}
    ...
    using the spatial \texttt{conStruct} model for $K=2$ through 4.
    }\label{populus_pies}
\end{figure}
...
as seen in Fig \ref{populus_sp_layer_covs},
...
For all values of $K$, the spatial model was preferred over the nonspatial model (Fig \ref{populus_xvals}) (strongly for $K<4$).
The nonspatial results were broadly concordant with those of the fastSTRUCTURE runs (Fig \ref{populus_fastStr}).
\gb{come back to this}.
...
The locations admixed individuals 
...
must use a possibly large number of discrete clusters
to describe this simple pattern
such continuous population differentiation.
...
However, discrete groupings are also often produced by important biological processes.
...
and a way to describe the contribution of each spatial layer to the model
(but caution against overly strict interpretation of these).
...
cluster assignment is not a replacement for absolute measures of genetic divergence.
...
individuals at a large spatial distance may be in the same layer but have very low pairwise relatedness.
...
drift terms across the layers from which they draw ancestry.
...
\begin{figure}
	...
	\caption{
	Plots showing the layer-specific parametric covariances 
	estimated for the \textit{Populus} data using 
	the nonspatial \texttt{conStruct} model run with $K=1$ through 6.
	Line colors are consistent with layer colors in Fig \ref{populus_nsp_pies}.
	Points are colored by the species they are a covariance between:
	black on black points are sample covariances between populations of \textit{Populus balsamifera};
	green on black points are sample covariances between \bals{} and \tri{};
	green on green points are sample covariances between \tri{} and \tri{}.
