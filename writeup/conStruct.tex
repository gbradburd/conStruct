...
\gb{support for the spatial model over the nonspatial model at a given value of $K$ 
indicates that isolation-by-distance is likely a feature of the data at that level.}

\paragraph{Choice of cluster number and cross-validation}
There are a number of reasons, both statistical and biological, why
there is no true (or right) number of clusters for real datasets. 
%There are a number of reasons, both statistical and biological, 
%why choosing a single, ``best" number of clusters to describe the data may not be desirable.
%, including ambiguity in what exactly it means for any given value of $K$ to be ``best."
We discuss these reasons further in the Discussion below. In practice
it is useful to explore the implications of clustering at a variety of
different levels (i.e. across multiple $K$) for biological
intepretation and downstream analyses. 
However, we also recognize that in many circumstances it is 
useful to know the number of clusters with highest statistical
support in the (current) dataset.  %, for example to focus su 
%However, we also recognize that in many circumstances 
i%t is either useful or convenient 
%to present only a single set of results from a clustering analysis with a particular number of clusters.
%ideally a number large enough to describe meaningful partitions in the data, 
%but not so large as to be difficult to parse visually.
...
Several approaches have been used as model choice criteria for the number of
clusters in \gc{population genetic} data, including: 
...
between several methods. %A comprehensive review of these approaches and their various merits is beyond the scope of this paper,
%but 

\gc{IS PREDICTION accuracy the right phrase?}
We choose to take a cross-validation approach.  We estimate the
posterior on the parameters of the model using a ``training" partition
of the data, in practice the training set is a random subset of $90\%$
of the loci. We then calculate the likelihood of the
``testing" subset, the remaining $10\%$ of the loci, averaged over the
posterior of the parameters. We repeat this procedure a number of
times, take an average of the relative log-likelihood over the runs at
sumarize each $K$. 
and selecting the model with the significantly best predictive accuracy
(see Section \ref{Xvalidation} for more on our cross-validation procedure).
This approach rewards models with larger values of $K$, which increase
the likelihood of the training partition, as they fit the data better,
but the improvement in the likelihood will plateau (or peak) as above
a certain $K$ we are fitting noise specific to the training data and
so not fitting the testing set any better, 
%up to the point that cluster membership is invoked to model noise specific to the training data, 
%at which point predictive accuracy decreases, 
%and models with additional clusters are penalized.
...
In addition, many \textit{Populus} species display patterns of isolation by distance, 
making the system well-suited to an analysis designed to tease apart 
genetic structure partitioned discretely across population (or species) clusters,
...
These species have a broad zone of overlap and are known to hybridize \citep{geraldes_etal_2014}.
...
(the majority of missing data were from drainages with only \textit{Populus balsamifera}).
...
\paragraph{Drift} 
We first provide a simple model of allele frequencies within a
cluster. Imagine a sample $i$ that draws all of its ancestry from cluster $k$.
We imagine that the allele frequency ($F$) at locus $\ell$ can be
written as a sum
\begin{equation}
...
\end{equation}
the first two terms are the ancestral allele frequency
$\epsilon_\ell$ shared by all samples and the deviation from that
ancestral mean due to drift in the ancestral population of $k$th cluster, $\Delta^{(k)}$,
which is shared by all samples within the cluster. The third term is
the deviation of sample $i$ away from the cluster mean due to the
spatial process of drift and migration within the cluster,
$\Delta^{(k,i)}$, the final term is the deviation specific to the $i$th sample, $\Delta^{(i)}$,
which captures drift not shared by all samples at the population level
(i.e., subpopulation-specific drift due to, e.g., inbreeding). We will
assume that these four deviations are all uncorrelated with each other.

If we have two populations $i$ and $j$ drawn from cluster $k$ their
covariance across loci will be 
\begin{equation}
\text{Var}(\epsilon) +  \text{Var}\left( \Delta^{(k)} \right) +
\text{Covar}(\Delta^{(k,i)}_{\ell},\Delta^{(k,j)}_{\ell}) + \delta_{i,j} \text{Var}\left( \Delta^{(i)} \right)\ \\
\end{equation}


%We rewrite this as 
%\begin{equation}
%\gamma +\mu^{(k)} + \delta_{i,j} \eta_{i}
%\end{equation}
%We set $\gamma = \text{Var}(\epsilon)$, the variance of allele
%frequencies due to drift
%in the ancestor of cluster $k$  $\mu^{(k)}=\text{Var}\left(
 % \Delta^{(k)} \right)$. We parameterize the spatial covariance of
%population $i$ and $j$ as
%\begin{equation}
%\gamma+  \text{Var}\left( \Delta^{(k)} \right) +
%\text{Covar}(\Delta^{(k,i)}_{\ell},\Delta^{(k,j)}_{\ell}) + \delta_{i,j} \text{Var}\left( \Delta^{(i)} \right)\ \\
%\end{equation}

%Under our non-spatial model we set
%$\text{Covar}(\Delta^{(k,i)}_{\ell},\Delta^{(k,j)}_{\ell}) =0$, such
%that  
...
admixture proportion of sample $i$ in cluster $k$ ($\sum\limits_{K}w^{(k)}_{i} = 1$),
which can be interpreted as the proportion of the genome in the $i$th
sample came from the $k$th cluster (or the probability that an allele
at a locus is drawn from $k$).
...
\end{equation}
Then the covariance, across loci, between $i$ and $j$ is
\begin{equation}
\Omega_{i,j} = \text{Var}(\epsilon) + \sum\limits_K w^{(k)}_iw^{(k)}_j
\left(
  \text{Var}\left( \Delta^{(k)} \right) +
\text{Covar}(\Delta^{(k,i)}_{\ell},\Delta^{(k,j)}_{\ell}) 	\right) +
\delta_{i=j} \text{Var}(\Delta_i)
\label{admixed_spatial_cov}
\end{equation}

Under our non-spatial model we assume that
$\text{Covar}(\Delta^{(k,i)}_{\ell},\Delta^{(k,j)}_{\ell})=0$ so that
the only additional convariance between $i$ and $j$ is due to the
drift in the ancestral population of our cluster (the variance of
which is $\mu_k$). Under our spatial
model we assume that the spatial covariance in allele frequencies
between $i$ and $j$ decays as a function of the geographic distance
between the pair $D_{i,j}$
\begin{equation}
\text{Covar}(\Delta^{(k,i)}_{\ell},\Delta^{(k,j)}_{\ell}) = \alpha^{(k)}_0 \times \left(\text{exp} \left(  -(\alpha^{(k)}_D D_{i,j})^{\alpha^{(k)}_2}\right) \right)	
\end{equation}
We note that this form is chosen for its flexibility and not because
it matches any explicit population-genetic model of isolation by
distance. 
%
%where
%
