...
\gc{Assuming that the} allele frequencies at each locus
...
number of degrees of freedom.  \gc{One possible solution, that we have
  not implemented yet, is to estimate an \emph{effective} number of loci 
...
All parameters are given (half-)Gaussian priors except for $\alpha_2$,
which is uniform on $(0,2)$ and $w$, which is an \gc{independent Dirichlet, of
  dimension $K$, for each individual}
...
by comparing their performance in a common framework. \gc{We also
  validate our claim that the results of our non-spatial method are a
  close match to those of STRUCTURE-based approaches.}

\paragraph{Choice of cluster number and cross-validation}
There are a number of \gc{reasons 
...
show statistically significantly greater predictive accuracy. \gc{DO
  WE DO t-test? IF NOT DROP THIS'}
