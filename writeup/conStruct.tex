...
More precisely, suppose that we pick a random locus uniformly,
pick a random ``reference'' allelic state from the two seen at that locus,
and in each sample draw a two random alleles without replacement,
recording $X_i = 1$ if the first allele drawn in sample $i$ matches the random reference,
and $X_i = 0$ otherwise;
and $\widetilde X_i = 1$ or $0$ for the second allele.
Then, 
\begin{equation} \label{eqn:first_allelic_cov}
    \widehat{\Omega}_{i,j} = \cov[X_i, \widetilde X_j] .
\end{equation}
...
Then $\widehat \Omega$ can be calculated as a covariance
...
These are $X_i$ and $\widetilde X_j$ in equation \eqref{eqn:first_allelic_cov},
