...
but we also recognize that in many circumstances it is either useful or convenient 
...
statistically comparing \texttt{conStruct} runs across different values of $K$.

Several approaches have been proposed to determine the number of clusters in the data, 
including: 
...
with various criteria on how to choose a single value (e.g., Evanno 2005);
...
A comprehensive review of these approaches and their various merits is beyond the scope of this paper,
but see \cite{verity_nichols2016} for an excellent discussion of and comparison between several.

We choose to take a cross-validation approach, 
estimating the parameters of the model using a ``training" partition of the data,
then calculating the likelihood of a separate, ``testing" partition of the data given those parameters,
repeating this procedure many times, 
and selecting the model with the significantly best predictive accuracy
(see Section \ref{Xvalidation} for a thorough discussion of our cross-validation procedure).
This approach rewards models with larger values of $K$, which increase the likelihood of the training partition,
