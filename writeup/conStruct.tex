...
(we discuss the diagonal of $\widehat{\Omega}_{i,j}$
further in \secref{allelic_cov}).

The choice of allele does not affect subsequent calculations, and so may be arbitrary.
The allelic covariance, $\widehat \Omega$, can be calculated as a covariance
(derived in \secref{allelic_cov}):
\begin{equation}
\widehat{\Omega}_{i,j} = 
    \frac{1}{L} \sum_{\ell=1}^L \left( (f_{i,\ell}-1/2) (f_{j,\ell}-1/2) \right).
...
For comparison,
we also analyzed each simulated dataset using ADMIXTURE \cite{ADMIXTURE}, using $K=1-7$, 
...
but each strongly favored the most cluster-rich model ran 
...
The discussion above and Equations \eqref{eqn:first_allelic_cov} and \eqref{allelic_covariance} 
give the form of the allelic covariance we model for the off-diagonal elements:
$\widehat{\Omega}$ for $i \neq j$.
The diagonal elements of $\widehat{\Omega}$ 
describe the within-population allelic variance:
$\var{\left(X_i\right)}$, where, as above, 
$X_i$ gives the randomly `flipped' coding of the genotypes in sample $i$,
to average over choice of reference allele.
The within-population allelic variance in sample $i$ is equivalent 
to the variance of a series of Bernoulli draws across loci, 
with a probability of `success' at locus $\ell$ given by 
the allele frequency in $i$ at $\ell$ ($F_{i,\ell}$), with probability 1/2, 
and $1-F_{i,\ell}$ with probability 1/2.
With sufficient data, $\widehat{\Omega} \approx 0.25$ for all samples.
