...
\plr{We could do this by talking about or including figure S28?}
}
...
\reply{We didn't follow exactly what this figure might look like, 
...
that are somewhat biologically realistic 
...
We have done ``sanity check'' simulations under the model itself,
...
As for the migration rates -- these are given in the text already \llname{ll:sim_params},
...
\reply{We have added more text comparing conStruct with Geneland and EEMS and BEDASSLE.}
...
\reply{We have edited that section for clarity and brevity.}
...
\reply{In practice, because those parameters can't be easily interpreted in a biological context, 
...
\reply{This is a nice idea, and one that we will definitely look into.
...
\reply{Yes, we think that's likely the best way to do it when linkage is known.
...
\reply{Good suggestion - we have now indicated that the method is available as a documented R package.}
